...
// import React, { useEffect, useState } from "react";
// import { api } from "../../api/api";
// import { Power, ChevronDown, Edit3, Trash2, Plus, Clock, CheckCircle2, XCircle, X, Check } from "lucide-react";
...
//       ...t,
//       status: (t.status || "PENDING").toUpperCase(), // 👈 handles lowercase from DB
//     }));
...
//     await fetchTasks(); // just refetch — safest approach
//     setTitle("");
//     setDescription("");
//   } catch (err) {
//     console.log(err);
...
//     try {
//       const response = await api.put(`/api/task/${taskId}`, { status: newStatus });
//       const updated = response.data.task;
//       setTask(prev =>
//         prev.map(t => t.id === taskId ? { ...t, ...updated } : t)
//       );
...
//         <button className="p-3 bg-white/5 rounded-full border border-white/10 text-white hover:bg-red-500/20 transition-all">
//           <Power size={20} />
//         </button>
//       </div>
...
//               Hello <span className="text-[#e086ff]">Srikresh</span>
//             </h1>
...
//               {task.length === 0 && (
//                 <p className="text-white/20 text-sm text-center py-10 italic">No tasks yet. Add one!</p>
//               )}

//               {task.map((item) => (
//                 <div key={item.id} className="bg-[#2a2424]/60 border border-white/5 rounded-[32px] p-6 space-y-4">

//                   {/* ── View Mode ── */}
//                   {editTask !== item.id ? (
//                     <>
//                       <div>
//                         <h3 className="text-[#e086ff] font-bold text-xl uppercase tracking-tighter">{item.title}</h3>
...
//       {/* FIXED DROPDOWN — outside all overflow containers */}
//       {activeMenu !== null && (
//         <div
...
// import React, { useEffect, useState } from "react";
// import { api } from "../../api/api";
// import { Power, ChevronDown, Edit3, Trash2, Plus, Clock, CheckCircle2, XCircle, X, Check, LogOut } from "lucide-react";
...
const Dashboard = () => {
  ...
  const handleStatusButtonClick = (e, itemId) => {
    ...
    const top = rect.bottom + dropdownHeight > screenHeight
      ? rect.top - dropdownHeight - 8
      : rect.bottom + 8;

    // Shift left if dropdown goes off right edge
    const left = rect.left + dropdownWidth > screenWidth
